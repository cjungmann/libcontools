...
void get_terminfo_values(void)
{
   ...
   while (ptr->value)
   {
      get_termcap_by_code((const char**)ptr->value, ptr->code);
      ++ptr;
   }
...
EXPORT void set_cursor_position(int row, int col)
{
...
EXPORT void get_cursor_position(int *row, int *col)
{
...
EXPORT void get_screen_size(int *rows, int *cols)
{
   (*rows) = 0;
   (*cols) = 0;
   struct winsize ws;
