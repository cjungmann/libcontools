 ...
 */
EXPORT const char *transform_keystroke(char *buff,
                                       int bufflen,
 ...
 */
EXPORT const char *transform_keystring(char *buff,
                                       int bufflen,
 ...
 * @return Pointer to @p buff if successful, NULL otherwise.
 */
EXPORT const char* get_transformed_keystroke(char *buff, int bufflen, const char *esc_str)
{
   memset(buff, 0, bufflen);

   char *raw_buff = (char*)alloca(bufflen);
   const char *raw_string = get_keystroke(raw_buff, bufflen);
   if (raw_string)
